...
export default function ScannerScreen() {
  ...
  async function scanWithOCR() {
    ...
    try {
      ...
      if (!contentType.includes('application/json')) {
        ...
        throw new Error(`Server error (${response.status}): ${raw.substring(0, 100)}`)
      }
      ...
      Alert.alert(
        'OCR Gagal',
        err.name === 'AbortError'
          ? 'Server AI tidak merespons (timeout). Pastikan layanan AI berjalan di port 9000.'
          : err.message
      )
    } finally {
      setOcrLoading(false)
  ...
  async function recommendMenu() {
    ...
    try {
      ...
      Alert.alert(
        'Rekomendasi Gagal',
        err.name === 'AbortError' ? 'Timeout - server tidak merespons.' : err.message
      )
    } finally {
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.actionGrid}>
          ...
          >
            ...
            <Text style={styles.actionDesc}>Foto langsung nota belanja</Text>
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            ...
            <Text style={styles.actionDesc}>Pilih foto dari galeri</Text>
          </TouchableOpacity>
        </View>
        <View style={styles.statusBar}>
          <View style={styles.statusDot} />
          <Text style={styles.statusText}>AI Service: PaddleOCR aktif (port 9000)</Text>
        </View>
      </SafeAreaView>
    )
  }
      ...
      <ScrollView contentContainerStyle={styles.resultContainer}>
        ...
        {menuRecs && (
          <View style={styles.card}>
            ...
            {menuRecs.notes && <Text style={styles.noteText}>{menuRecs.notes}</Text>}
          </View>
        )}
      </ScrollView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  actionDesc: { color: '#6b7280', fontSize: 12, textAlign: 'center' },
  statusBar: {
